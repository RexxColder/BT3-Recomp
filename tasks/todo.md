# Launcher/Overlay pre-release: Post-FX fuera + escala interna por resoluciÃ³n â€” 2026-09-13
...
- El launcher y el overlay exponen un toggle "Post-FX" y un combo "Internal Resolution"
  ...
  reescalado pasa a ser built-in derivado de la resoluciÃ³n elegida.
- Post-FX estaba OFF por defecto en todos los puntos (Settings=false, SettingsManager=false,
  `GsGpuRenderer::postfxEnabled()` default false, `PS2X_POSTFX` NO baked en main.cpp) â†’
  es seguro borrar hasta la env var.
- Mapeo aprobado por el usuario: 1x = 720p, 2x = >720p hasta 1080p, 3x = â‰¥1440p en adelante.
  Se aÃ±ade preset 4K (3840x2160 â†’ 3x). El 4x queda deshabilitado (nada lo genera) hasta que funcione.

## Elementos verificables
- [x] Header compartido `include/runtime/ps2_render_scale.h` con `ps2xRenderScaleForHeight`.
- [x] Post-FX: 0 refs a `postfx`/`PS2X_POSTFX` en cÃ³digo (solo comentarios descriptivos de draw classes).
- [x] Launcher: sin combo "Internal Resolution"; lista de resoluciÃ³n con 4K; escala derivada
      en cambio de Window Size y al inicializar (re-derivaciÃ³n de INIs viejas).
- [x] Overlay: sin combo "Internal Resolution"; 4K en Window Size; escala derivada en
      resoluciÃ³n/fullscreen (live en paraLLEl-GS).
...
- [ ] CompilaciÃ³n de ps2xRuntime (raylib/imgui por FetchContent; pendiente â€” el usuario pidiÃ³
...
- Balance de llaves en `ps2_gs_gpu_renderer.cpp`, `ps2_settings_overlay.cpp`, `tab_video.cpp`:
  idÃ©ntico al HEAD (el +1 del overlay es un falso positivo del checker pre-existente).
- El bloque runtime de Post-FX quedÃ³ como gates incondicionales (comportamiento = default OFF anterior).

## Notas
- El bloque runtime `if (!s_postfx) {â€¦gatesâ€¦}` se convierte a gates SIEMPRE activos
  (comportamiento visual idÃ©ntico al default actual, validado a 30fps).
- `ps2xRenderScaleForHeight(h)`: `h<=720 â†’ 1`; `h<=1080 â†’ 2`; `else â†’ 3`.
---

# ExtracciÃ³n de archivos con nombres reales ("propper names and formats")
...
- El `.db` (`debug_font_PS2_.db`) NO es una lista: es un **font/glifo** (bytes de
  pÃ­xeles). La lista de carga real del ISO es `PZS3US.DIR` (solo los 8 archivos tope)
...
- Comunidad: los **AFL nuevos** de ViveTheModder (`vitetheModder.github.io`, repo Apache-2.0)
  traen el nombre real + formato de cada archivo, alineados por Ã­ndice del AFS:
  - PZS3US1: 3399 archivos â†’ `.pak` (2327), `.cdbt` (658), `.dbt` (323), `.gsc` (50), `.cpak` (41)
  - PZS3US2: 65201 archivos â†’ `.adx` (65201, todo sonido/voces)
  - 0 duplicados, 0 nombres vacÃ­os, 0 no-ASCII â†’ formatos reales legibles.
...
- [x] `loadNameTable` parsea binario AFL (valida count == entries) + legacy texto (`idx\tname`);
      prioridad: `.afl` real â†’ tabla embebida â†’ fallback.
- [x] Reemplazar `assets/PZS3US1.afl`, `assets/PZS3US2.afl` con los AFL nuevos.
- [x] NOTICE Apache-2.0 + crÃ©dito ViveTheModder en README.
...
- [x] Re-extracciÃ³n: PZS3US1 (3399) + PZS3US2 (65201) byte-idÃ©nticos vs AFS (0 mismatches).
- [x] Regenerar `portable/data/DATA` (idx v3 + nombres reales): PZS3US0/1/2 OK.
- [x] ValidaciÃ³n contrato runtime: idx v3 resuelve cada slot a `folder/<name>` byte-idÃ©ntico (python).
- [ ] Rebuild docker + package + redeploy al Escritorio (aÃºn con runner build v2 en portable).
...
# Port launcher â†’ Windows/macOS (input multiplataforma) â€” 2026-09-09
...
quedÃ³ abandonado por decisiÃ³n del usuario; la validaciÃ³n Windows es dual-boot.
...
- [x] `tab_bindings`: `evKeyToRaylib` â†’ `qtKeyToRaylib` + eventFilter Qt (captura teclado).
- [x] Launcher CMake: GLFW FetchContent 3.4, `if(NOT MSVC)` en `-Wall -Wextra`, POST_BUILD copia `background.png`/`icon.png`.
- [x] `_WIN32`: `bt3-runner.exe`, LD_LIBRARY_PATH solo POSIX, `MoveFileEx` para rename atÃ³mico en `pad_config_reader.cpp`, setPermissions no-op en Windows (`extract_worker.cpp`).
...
- [x] Fix `entrypoint.sh` SIGPIPE (`find | head -40` + pipefail) â€” el clone local aÃºn no tenÃ­a el fix de PR #6.
- [x] `entrypoint.sh` + `package.sh` ahora incluyen `data/` en stage/tarball (faltaba; el tarball salÃ­a sin el juego).
- [x] Flujo docker de 0: runner+launcher compilan (glibc floor 2.35), tarball 1.9G con `data/` + `install game.sh` + `.sha256`.
- [x] descomprimir â†’ `install game.sh` (HOME temporal) â†’ .desktop vÃ¡lido â†’ Launcher corre â†’ savedata preservado en re-install.
...
# setup.py multi-stage rewrite â€” 2026-09-18
...
# Build de 0 con setup.py (verificacion end-to-end) — 2026-09-18

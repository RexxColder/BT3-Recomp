...
static inline __m128 ps2VuSat(__m128 v)
{
    ...
    const __m128i isBad = _mm_cmpeq_epi32(_mm_and_si128(iv, expMask), expMask); // Inf/NaN lanes
    const __m128i sat = _mm_or_si128(_mm_and_si128(iv, _mm_set1_epi32((int)0x80000000u)),
                                     _mm_set1_epi32(0x7F7FFFFF)); // +/-FLT_MAX, sign kept
    return _mm_castsi128_ps(_mm_or_si128(_mm_and_si128(isBad, sat), _mm_andnot_si128(isBad, iv)));
}
#define PS2_VADD(a, b) ps2VuSat(_mm_add_ps((__m128)(a), (__m128)(b)))
#define PS2_VSUB(a, b) ps2VuSat(_mm_sub_ps((__m128)(a), (__m128)(b)))
...
// FPU (COP1) operations
// The EE FPU is NOT IEEE-754: it has no NaN/Inf — results saturate to +/-Fmax on
// overflow, and DIV by zero yields +/-Fmax. Plain host float math lets Inf/NaN form
// and propagate (BT3: hair-physics divergence to 1e22 + characters vanishing on NaN
// MVPs, 2026-07-23). ps2FpuSat() restores the hardware saturation on every result.
static inline float ps2FpuSat(float f)
{
    ...
    if ((u & 0x7F800000u) == 0x7F800000u) // IEEE Inf/NaN (exponent 255)
    {
        ...
        static const bool s_on = [](){ const char *v = ::getenv("PS2X_FPU_SAT"); return !(v && v[0] == '0'); }();
        if (s_on)
        {
...
#define FPU_ADD_S(a, b) ps2FpuSat((float)(a) + (float)(b))
#define FPU_SUB_S(a, b) ps2FpuSat((float)(a) - (float)(b))
#define FPU_MUL_S(a, b) ps2FpuSat((float)(a) * (float)(b))
#define FPU_DIV_S(a, b) ps2FpuSat((float)(a) / (float)(b))

...
{   // [decpool] the pool thread's twin of the inline decode: the job's register/palette snapshot plus the VRAM span it
    // copied into this thread's scratch. gs == nullptr keeps decodeTexRGBA on its GS-free fast paths.
    TexDecodeSrc src;
    src.tex0 = &job.tex0; src.clamp = job.clamp; src.vram = scratch; src.vramSize = vramSize;
    src.clut = job.clut; src.clutKey = job.clutKey; src.texa = &job.texa; src.texclut = &job.texclut;
    src.subDxW = job.subDxW; src.subDx0 = job.subDx0;
    decodeTexRGBA(nullptr, src, job.texW, job.texH, job.rawAlphaDec, job.texKey, subW, rgba);
}
...
bool GSRasterizer::recordSpriteGPU(RecInput &in)
{
    ...
    if (tme)
    {
        ...
        {
            texNeedDecode = true;
            swapPending = true;   // a finished async replacement: must re-decode, never take a shortcut
        }
        ...
        else if (texNeedDecode && !gaServedRead && GsGpuRenderer::decPoolWants(ctx.tex0.psm))
        {   // [decpool] the decode leaves the kick worker: snapshot what the inline decode would read (registers, the
            // built palette, the source VRAM span) and hand it to the decode pool. A placeholder cache entry stamped

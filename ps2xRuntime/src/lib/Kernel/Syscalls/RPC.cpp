...
namespace ps2_syscalls
{
    ...
    void SifLoadModule(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // default is the validated set. Lets us isolate which module breaks the boot.
        {
            ...
            std::string set = "all";   // every integrated IRX runs native; see tools/iop/README.md
    ...
    void SifBindRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        {
            static std::atomic<uint32_t> s_bindLog{0};
            uint32_t n = s_bindLog.fetch_add(1);
            if (n < 80u)
                std::cerr << "[bind] sid=0x" << std::hex << rpcId
                          << " client=0x" << clientPtr
                          << " ra=0x" << getRegU32(ctx, 31)
                          << std::dec << " tid=" << g_currentThreadId << std::endl;
        }
    ...
    void SifCallRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        uint32_t mode = getRegU32(ctx, 6);
        {
            static std::atomic<uint32_t> s_rpcDbg{0};
            uint32_t sidLog = 0;
        ...
        // BT3 sound-init trace (env PS2X_RPCLOG): log every sound-sid / DTX URPC call
        // with the poll-relevant fields + the stuck sound-ready counter 0x2c9f14, so we
        // can see WHICH command the engine polls and whether its end-callback fired.
        {
            static const bool s_rpcLog = [](){ const char *v = std::getenv("PS2X_RPCLOG"); return v && v[0] && v[0] != '0'; }();
            if (s_rpcLog && (boundSidHint == 0x90000200u || sid == 0x90000200u || isDtxSid || isDtxUrpc))
            {
                static std::mutex s_lm;
                static std::map<uint32_t, uint32_t> s_perRpc;
                std::lock_guard<std::mutex> lk(s_lm);
                uint32_t &c = s_perRpc[rpcNum];
                ++c;
                if (c <= 6u || (c % 400u) == 0u)
                {
                    uint32_t s0 = 0u, s1 = 0u, r0 = 0u;
                    if (sendBuf) (void)readRpcU32(sendBuf, s0);
                    if (sendBuf) (void)readRpcU32(sendBuf + 4u, s1);
                    if (recvBuf) (void)readRpcU32(recvBuf, r0);
                    uint32_t cnt = *reinterpret_cast<uint32_t *>(rdram + (0x2c9f14u & 0x1FFFFFFFu));
                    std::cerr << "[rpclog] sid=0x" << std::hex << (boundSidHint ? boundSidHint : sid)
                              << " rpc=0x" << rpcNum << " urpcCmd=" << std::dec << dtxUrpcCommand
                              << " mode=0x" << std::hex << mode
                              << " nowait=" << std::dec << ((mode & kSifRpcModeNowait) ? 1 : 0)
                              << " endFn=0x" << std::hex << endFunc
                              << " hleIop=" << std::dec << (handledByIop ? 1 : 0)
                              << " cb=" << (callbackInvokedForDebug ? 1 : 0)
                              << " res=0x" << std::hex << resultPtr
                              << " send=[0x" << s0 << ",0x" << s1 << "]"
                              << " recv0=0x" << r0
                              << " cnt(2c9f14)=" << std::dec << cnt
                              << " n=" << c << std::endl;
                }
            }
        }

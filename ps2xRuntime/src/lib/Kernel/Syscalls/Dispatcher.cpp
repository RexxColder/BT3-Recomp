...
namespace ps2_syscalls
{
    ...
    {
        {
            static std::atomic<uint32_t> s_scDbg{0};
            const uint32_t n = s_scDbg.fetch_add(1);
            if (n >= 20000u && n < 20120u) // steady-state window
                std::cerr << "[sc] tid=" << g_currentThreadId << " num=0x" << std::hex << syscallNumber
                          << " a0=0x" << getRegU32(ctx, 4) << " a1=0x" << getRegU32(ctx, 5) << std::dec << std::endl;
        }

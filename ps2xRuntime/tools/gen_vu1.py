...
def emit_pair_body(out, k, lo, up, pairs, inline_mode):
    ...
    out.append(f'        if (cyc++ >= maxCycles) {{ st.pc = 0x{pc:x}u; return; }}')
    ...
    up_code = '' if upper_is_nop(up) else f'if (!vujit::jitUpper<0x{up:08x}u>(st)) {{ vu.jitSlowUpper(0x{up:08x}u); fb = true; }}'
    if is_branch:
        ...
        lo_code = f'if (!vujit::jitLower<0x{lo:08x}u>(st, vuData, dataSize)) {{ vu.jitSlowLower(0x{lo:08x}u, vuData, dataSize, gs, memory, 0x{up:08x}u); fb = true; }}'
    if loi:
...
def gen_program(name, image, npairs):
    ...
    for k in range(npairs):
        ...
        if ebit:
            ...
            out.append(f'        st.ebit = true; st.pc = 0x{8 * (k + 2):x}u; return;')
        elif is_branch:
            out.append('        if (br) {')
            if nxt < npairs:
                nlo, nup = pairs[nxt]

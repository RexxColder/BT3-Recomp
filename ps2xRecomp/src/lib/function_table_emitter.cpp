...
namespace ps2recomp
{
    ...
    std::string FunctionTableEmitter::emit(const std::vector<Function> &functions, const std::map<uint32_t, std::string> &stubs)
    {
        ...
        if (cg.m_bootstrapInfo.valid)
        {
            ...
            if (entryTarget.empty())
            {
                // Auxiliary modules (e.g. overlay binaries recompiled with a partial
                // function list) may have no function at the ELF entry point; skip the
                // bootstrap registration instead of failing the whole generation.
                std::cout << "  [warning] output - no function at ELF entry 0x" << std::hex
                          << cg.m_bootstrapInfo.entry << std::dec
                          << "; skipping entry registration." << std::endl;
            }
            else
            {

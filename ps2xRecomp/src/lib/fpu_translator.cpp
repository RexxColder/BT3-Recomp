...
namespace ps2recomp
{
    ...
    std::string FpuTranslator::translate(const Instruction &inst)
    {
        ...
        case COP1_S:
            ...
            case COP1_S_DIV:
                // EE FPU has no Inf/NaN: DIV by zero saturates to +/-Fmax, and all results clamp.
                return fmt::format("if (ctx->f[{}] == 0.0f) {{ ctx->fcr31 |= 0x100000; /* DZ flag */ "
                                   "ctx->f[{}] = copysignf(3.4028235e38f, ctx->f[{}] * 0.0f); }} "
                                   "else ctx->f[{}] = ps2FpuSat(ctx->f[{}] / ctx->f[{}]);",
                                   ft, fd, fs, fd, fs, ft);
            case COP1_S_SQRT:
                // EE SQRT.S is "sqrt.s fd, ft" — the SOURCE is the FT field, not FS (which is
                // zero in the encoding). Using fs computed sqrt(f0) at 90 BT3 sites (stale-
                // register garbage into quaternions/lengths — hair, IK, geometry).
            ...
            case COP1_S_RSQRT:
                // fd = fs / sqrt(|ft|) — two operands (the old emission ignored ft); saturates.
                return fmt::format("ctx->f[{}] = ps2FpuSat(ctx->f[{}] / sqrtf(fabsf(ctx->f[{}])));", fd, fs, ft);
            case COP1_S_ADDA:

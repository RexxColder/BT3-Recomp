...
namespace ps2recomp
{
    ...
    {
        std::string code = VuTranslator(*this).translate(inst);
        // VU register vf0 is hardwired read-only to (0,0,0,1) on real hardware; writes to it
        // are silently discarded. Games use vf0 as a dummy destination for flag-only ops
        // (e.g. `vsub vf0,...`). Executing those as real writes corrupts the vf0 constant,
        // which then poisons the identity basis derived from it (vmr32 vf1,vf0; ...) and
        // collapses every transform matrix. Redirect any write whose destination is vf0 to a
        // throwaway sink; reads of vf0 (as a source operand) are unaffected.
        for (std::string::size_type p = code.find("ctx->vu0_vf[0] = ");
             p != std::string::npos; p = code.find("ctx->vu0_vf[0] = ", p))
        {
            code.replace(p, std::string("ctx->vu0_vf[0]").size(), "ctx->vu0_vf_dummy");
            p += std::string("ctx->vu0_vf_dummy").size();
        }
        return code;
    }

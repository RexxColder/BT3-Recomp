...
namespace ps2recomp
{
    ...
    std::string CodeGenerator::translateVU_VDIV(const Instruction &inst)
    {
        ...
        // Hardware DIV-by-zero = +/-Fmax by numerator sign. Safe now that the PS2_V* vector
        // macros saturate Inf/NaN to +/-Fmax (ps2VuSat) like real VU hardware — Fmax values
        // flow through multiplies (0*Fmax=0) instead of exploding to Inf/NaN.
        return fmt::format("{{ float fs = _mm_cvtss_f32(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(0,0,0,{}))); float ft = _mm_cvtss_f32(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(0,0,0,{}))); ctx->vu0_q = (ft != 0.0f) ? (fs / ft) : ((fs >= 0.0f) ? 3.4028235e38f : -3.4028235e38f); }}", fs_reg, fs_reg, fsf, ft_reg, ft_reg, ftf);
    }
    ...
    std::string CodeGenerator::translateVU_VSQRT(const Instruction &inst)
    {
        uint8_t ftf = inst.vectorInfo.ftf;
        uint8_t ft_reg = inst.rt;
        return fmt::format("{{ float ft = _mm_cvtss_f32(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(0,0,0,{}))); ctx->vu0_q = sqrtf(fabsf(ft)); }}", ft_reg, ft_reg, ftf);
    }

    std::string CodeGenerator::translateVU_VRSQRT(const Instruction &inst)
    {
        // Q = FS / sqrt(|FT|) (two operands; FT==0 saturates like DIV-by-zero).
        uint8_t fsf = inst.vectorInfo.fsf;
        uint8_t ftf = inst.vectorInfo.ftf;
        uint8_t fs_reg = inst.rd;
        uint8_t ft_reg = inst.rt;
        return fmt::format("{{ float fs = _mm_cvtss_f32(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(0,0,0,{}))); float ft = sqrtf(fabsf(_mm_cvtss_f32(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(0,0,0,{}))))); ctx->vu0_q = (ft != 0.0f) ? (fs / ft) : ((fs >= 0.0f) ? 3.4028235e38f : -3.4028235e38f); }}", fs_reg, fs_reg, fsf, ft_reg, ft_reg, ftf);
    }
    ...
    std::string CodeGenerator::translateVU_VOPMSUB(const Instruction &inst)
    {
        ...
        uint8_t vft = inst.rt;
        uint8_t dest_mask = inst.vectorInfo.vectorField;
        // True cross-product component rotation (OPMSUB): vd.xyz = ACC.xyz - (vs.yzx * vt.zxy)
        return fmt::format("{{ __m128 mul_res = PS2_VMUL(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(3,0,2,1)), _mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(3,1,0,2))); "
                           "__m128 res = PS2_VSUB(ctx->vu0_acc, mul_res); "
                           "__m128i mask = _mm_set_epi32({}, {}, {}, {}); "
                           "ctx->vu0_vf[{}] = _mm_blendv_ps(ctx->vu0_vf[{}], res, _mm_castsi128_ps(mask)); }}",
                           vfs, vfs, vft, vft,
    ...
    std::string CodeGenerator::translateVU_VOPMULA(const Instruction &inst)
    {
        ...
        uint8_t vft = inst.rt;
        uint8_t dest_mask = inst.vectorInfo.vectorField;
        // True cross-product component rotation (OPMULA): ACC.xyz = vs.yzx * vt.zxy
        return fmt::format("{{ __m128 res = PS2_VMUL(_mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(3,0,2,1)), _mm_shuffle_ps(ctx->vu0_vf[{}], ctx->vu0_vf[{}], _MM_SHUFFLE(3,1,0,2))); ctx->vu0_acc = _mm_blendv_ps(ctx->vu0_acc, res, {}); }}",
    ...
    std::string CodeGenerator::translateVU_VLQI(const Instruction &inst)
    {
        uint8_t vis = inst.rd;
        uint8_t dest_mask = inst.vectorInfo.vectorField;
        // VU0-macro LQ/SQ family addresses VU0 data memory (4KB at 0x11004000, 256 qwords),
        // not raw EE RAM; vi regs are 16-bit. Loads take the vf in the ft slot / vi in the fs
        // slot (rt/rd here); stores are the reverse: data vf in the fs slot, addr vi in ft.
        return fmt::format("{{ uint32_t addr = 0x11004000u + (((uint32_t)(ctx->vi[{}] & 0xFFu)) << 4); "
    ...
    {
        uint8_t vfs = inst.rd;  // data source vf (fs slot)
        uint8_t vit = inst.rt;  // address vi (ft slot)
        uint8_t dest_mask = inst.vectorInfo.vectorField;
        return fmt::format("{{ uint32_t addr = 0x11004000u + (((uint32_t)(ctx->vi[{}] & 0xFFu)) << 4); "
    ...
    std::string CodeGenerator::translateVU_VLQD(const Instruction &inst)
    {
        uint8_t vis = inst.rd;
        uint8_t dest_mask = inst.vectorInfo.vectorField;
        return fmt::format("{{ ctx->vi[{}] = (uint16_t)(ctx->vi[{}] - 1u); "
                           "uint32_t addr = 0x11004000u + (((uint32_t)(ctx->vi[{}] & 0xFFu)) << 4); "
                           "__m128 res = _mm_castsi128_ps(READ128(addr)); "
                           "__m128i mask = _mm_set_epi32({}, {}, {}, {}); "
                           "ctx->vu0_vf[{}] = _mm_blendv_ps(ctx->vu0_vf[{}], res, _mm_castsi128_ps(mask)); }}",
                           vis, vis,

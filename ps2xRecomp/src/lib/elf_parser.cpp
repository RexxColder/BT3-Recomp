...
namespace ps2recomp
{
    ...
    std::vector<Function> ElfParser::extractFunctions() const
    {
        ...
        {
            // [r3000] Address 0 is valid for an IRX (code at vaddr 0); only reject it when no
            // executable section starts there (the EE's code lives at 0x100000+).
            const bool zeroAddressOk = extra.start != 0 ||
                                       FindFunctionSectionByAddress(m_sections, 0) != nullptr;
            if (!zeroAddressOk || extra.end <= extra.start || extra.name.empty() || IsAutoGeneratedName(extra.name))
            {
                continue;
            }
        ...
        {
            // [r3000] A valid IRX function can live at address 0 (code at vaddr 0).
            if (newFunction.start == 0 && !FindFunctionSectionByAddress(m_sections, 0))
            {
                return;
            }

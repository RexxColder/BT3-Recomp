...
namespace ps2recomp
{
    ...
    std::string VuTranslator::translate(const Instruction &inst)
    {
        ...
        case COP2_CFC2:
        {
            ...
            case VU0_CR_CLIP2:
                // COP2 control register 22 is the Q register on real hardware (16=STATUS,
                // 17=MAC, 18=CLIP, 20=R, 21=I, 22=Q, 23=P). Reading it as clip flags made
                // every sceVu0-style vector length return garbage (BT3 movement freeze bug).
                return fmt::format("{{ uint32_t qbits; std::memcpy(&qbits, &ctx->vu0_q, sizeof(qbits)); SET_GPR_U32(ctx, {}, qbits); }}", rt);
            case VU0_CR_P:
        ...
        case COP2_CTC2:
        {
            ...
            case VU0_CR_CLIP2:
                // COP2 control register 22 = Q register (see CFC2 note above).

...
Both layouts work. A flat `data/` looks like this, with `IRX/` still needing
its own folder because the game loads modules from that exact path:

```
data/
├── DBZP.BIN  PZS3US.DIR  PZS3US{0,1,2}.{ALG,idx}  ZS3{USED,USOP}.{PSS,ADX}
├── PZS3US0/  PZS3US1/  PZS3US2/       # the converted AFS containers
├── IRX/                                # kernel modules, loaded at runtime
...
The game asks for it: the log shows `[iop-run] cannot load IRX:
data/IRX/CRI_ADXI.IRX`, and `[iop-run] CRI_ADXI.IRX: entry ... (native)` once
it is there. The audio driver comes from that folder. It is small (0.7 MB).

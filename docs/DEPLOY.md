...
├── assets/music/music.flac # menu theme, only with setup.py --with-music (copyrighted audio)
├── fps60_sites.txt         # 60 fps pacing table, read next to the runner on first enable
├── savedata/
│   ├── settings.toml    # user settings ([logging], [video], [netplay], …)
│   └── pad_p1.conf / pad_p2.conf       # front-end bindings
├── data/                   # the game (from your ISO) + NETPLAY.BIN; see below
...
`data/NETPLAY.BIN` is the one file in `data/` that the deploy writes rather than
the wizard: the Dragon Net menu art, as a single blob. See below.
...
### `data/` can be flat, and how the resolvers were taught

The installed tree does **not** have to mirror the disc. The guest asks for disc
paths (`\DATA\PZS3US1.AFS;1`, `\BIN\DBZP.BIN`) and two host resolvers answer:

- **AFS-backed data** — `Kernel/Stubs/Helpers/Support.h`: a request for a
  `*.afs` looks for `<host>/DATA/PZS3US1.idx` + directory `<host>/DATA/PZS3US1/`.
  If that pair is missing it retries the same pair at the CD root, which is what
  makes a flat install work. Without the retry there is nothing else to fall
  back to: the `.AFS` is deleted once the folder exists.
- **FIO file I/O** — `Kernel/Syscalls/Helpers/Runtime.h`: `resolvePs2HostPath()`
  wraps `translatePs2Path()` and, when the exact host path is absent, retries the
  file by name at the CD root. The FIO syscalls have no index, so without this
  every open outside the root fails (862 `fioOpen` failures when measured).

Both layouts work. `IRX/` may sit in its own folder or flat at the root, because
`PS2Runtime::resolveIopModulePath()` tries `<cdRoot>/IRX/<mod>` and then
`<cdRoot>/<mod>`, anchored to the CD root rather than the process directory:
...
├── NETPLAY.BIN                         # Dragon Net menu art, one blob (not from the disc)
├── SLUS_216.78                         # the boot ELF
└── SYSTEM.CNF
```

`NETPLAY.BIN` is the only file here that does not come from the disc. The deploy
writes it under its final name (`games/bt3/dragonnet_assets.bin`, packed by
`tools/iop/dragonnet_assets.py`) and the menu reads its PNGs by name, so 19 loose
images cost one file. A hand-run build has no deploy to do that, so CMake stages
the source blob next to the runner and the runtime installs it into `data/` on
first use (`dragonnet::ensureInstalled()`); the reader prefers `data/NETPLAY.BIN`
and only falls back to the staged copy. Settings → Varios → NETPLAY reports which
of the three states the install is in.
...
runner; it only feeds `tools/iop/ioprp.py` for inspection.

The Dragon Net entry's own track, `assets/DragonNet/music/netmenu.mp3`, follows the
menu theme's rule: it is copyrighted audio, so it is a local file and only rides
along with `setup.py --with-music`. The runtime resolves it against the runner's
own directory, so it works from any working directory, and `PS2X_NETMENU_MUSIC`
overrides the path.

`data/texcache.bin` is the opposite: generated by playing
(`ps2_runtime.cpp`, `ps2texcache::load()`), it grows past a gigabyte, and it is
safe to delete at any time. The toggle is `video.texcache` in `settings.toml`.

### The Dragon Net Menu is a toggle, and it is only a page

`[netplay] dragon_net_menu` in `settings.toml` is what Settings → Varios →
NETPLAY writes, and `main.cpp` turns it into `PS2X_NET_MENU` after the front-end
has had its chance to save (with `overwrite=0`, so an explicit `PS2X_NET_MENU=0`
in the environment still wins). The flag is read once, on the first frame, so the
change applies on the next launch rather than mid-session.

What it turns on is the post-generation patch set that reveals the retail main
menu's hidden "Network Battle" plate and hosts the custom page
(`games/bt3/apply_overlay_patches.py`, `ps2x_net_menu.cpp`). It is **not** a
connection: the page's room list is mock data, the Join button only logs, and the
transport itself (`PS2X_NET` / `PS2X_NET_LISTEN`) stays env-only because hosting
or joining is a session, not a preference. The page is also still behind a raw
entry test — entering the row reaches the real Duel, but the 2D screen is not
raised from that path yet.

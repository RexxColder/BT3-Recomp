...
### The Dragon Net menu entry is retired

The custom page behind the main menu's hidden "Network Battle" plate was removed
on 2026-09-25. Nothing in the build ships it, nothing in the settings offers it,
and the main menu is back to stock: `PS2X_REVEAL_HIDDEN_MENU_ENTRY` now defaults
to `0`, so the plate stays hidden rather than leading nowhere. The reasoning and
everything that was measured while trying to make it work is in
[`NEW-NETMENU.md`](NEW-NETMENU.md), which is kept as the record.

The netplay **transport** is a different thing and is untouched: `ps2_netplay.cpp`
(lockstep over UDP) and `PS2X_NET` / `PS2X_NET_LISTEN` still exist and are still
env-only, because hosting or joining is a session and not a preference.

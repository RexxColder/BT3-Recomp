...
### `data/` layout is load-bearing: do not flatten it

The installed tree mirrors the disc, and that is not cosmetic. The guest asks for
disc paths (`\DATA\PZS3US1.AFS;1`, `\BIN\DBZP.BIN`) and two different host
resolvers answer them, with **no shared fallback**:

- **AFS-backed data** — `Kernel/Stubs/Helpers/Support.h`: a request for a
  `*.afs` looks for the exact pair `<host>/DATA/PZS3US1.idx` + directory
  `<host>/DATA/PZS3US1/`. If that pair is not there it falls through to the
  loose-file index, which looks for a *file* of that name — and the `.AFS` was
  deleted once the folder existed, so nothing is found.
- **FIO file I/O** — `Kernel/Syscalls/Helpers/Runtime.h`: `translatePs2Path()`
  builds the host path as `<cdRoot>/<guest path>` and `fioOpen()` opens it as-is.

So flattening `data/DATA/*` up one level breaks the game unless both resolvers
grow a fallback. Measured on a real install (2.7 GB, 68.6k files):

| layout | `sceCdSearchFile failed` | `fioOpen` failures | fps |
|---|---|---|---|
| disc layout | 0 | 0 | 6.8 – 8.8 |
| flattened | 0 | 862 (until the fallback) | 4.9 – 10.8 |

The flattening is **not** faster: the A/B puts both in the same 5–10 fps band.
It was reverted.

### `data/IRX/` is required at runtime

Earlier claims that `IRX/` is diagnostic-only are wrong. The game itself asks
for it: the log shows `[iop-run] cannot load IRX: data/IRX/CRI_ADXI.IRX`, and
`[iop-run] CRI_ADXI.IRX: entry ... (native)` when it is there. The audio driver
comes from that folder. It is small (0.7 MB) and stays.

`data/texcache.bin` is the exception: it is generated by playing
(`ps2_runtime.cpp`, `ps2texcache::load()`), grows to ~1.8 GB, and is safe to
delete at any time; the toggle is `video.texcache` in `settings.toml`.
